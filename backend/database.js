...
const uri =
  "mongodb+srv://sarthaksrivastavabca21:Sarthak1234@cluster0.krtvbxs.mongodb.net/Cyrpto";
...
const checklogin = async (object) => {
  const collname = await getCollection("Usersignup");
  const see = await collname.findOne({
    email: object.email,
...
const Updatepass = async (obj, mail) => {
  ...
  const collname = await getCollection("Usersignup");
  const ans = await collname.updateOne(
    { email: email },
...
//CrMughav@1017 pass of CryptoMart1011@gmail.com pass: eywm ijom zgse rani
const show = async () => {
...
const update = async (id, object) => {
  const collName = await getCollection("transaction");
  return await collName.findOneAndUpdate(
    { _id: ObjectId(id) },
    { $set: object }
  );
};
